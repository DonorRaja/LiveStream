...
/**
  If true, this handler is expecting to be acked. Call \code
  with(_: SocketData...)
  \endcode to ack.
*/
...
/**
  ...
  \param items An array of items to send when acking. Use \code
  []
  \endcode to send nothing.
...
/**
  \code
  true
  \endcode if this engine is closed.
*/
@property (nonatomic, readonly) BOOL closed;
/**
  \code
  true
  \endcode if this engine is connected. Connected means that the initial poll connect has succeeded.
*/
...
*/
@property (nonatomic, readonly, strong) OS_dispatch_queue * _Nonnull engineQueue;
/**
...
/**
  When \code
  true
  \endcode, the engine is in the process of switching to WebSockets.
*/
@property (nonatomic, readonly) BOOL fastUpgrade;
/**
  When \code
  true
  \endcode, the engine will only use HTTP long-polling as a transport.
*/
@property (nonatomic, readonly) BOOL forcePolling;
/**
  When \code
  true
  \endcode, the engine will only use WebSockets as a transport.
*/
@property (nonatomic, readonly) BOOL forceWebsockets;
/**
  If \code
  true
  \endcode, the engine is currently in HTTP long-polling mode.
*/
@property (nonatomic, readonly) BOOL polling;
/**
  If \code
  true
  \endcode, the engine is currently seeing whether it can upgrade to WebSockets.
*/
...
/**
  If \code
  true
  \endcode, then the engine is currently in WebSockets mode.
*/
...
/**
  ...
  If \code
  true
  \endcode we might have to fix utf8 encoding.
...
/**
  ...
  \endcode for transport specific methods.
*/
...
/**
  \code
  true
  \endcode if there is an outstanding poll. Trying to poll before the first is done will cause socket.io to
  disconnect us.
...
/**
  \code
  true
  \endcode if there is an outstanding post. Trying to post before the first is done will cause socket.io to
  disconnect us.
  <em>Do not touch this directly</em>
*/
...
/**
  If \code
  true
  \endcode the engine will attempt to use WebSocket compression.
*/
...
/**
  \code
  true
  \endcode If engine’s session has been invalidated.
*/
...
/**
  ...
  <em>NOTE</em>: The client is not thread/queue safe, all interaction with the socket should be done on the \code
  handleQueue
  \endcode
...
/**
  If \code
  true
  \endcode then every time \code
  connect
  \endcode is called, a new engine will be created.
*/
...
/**
  ...
  called on.
*/
@property (nonatomic, strong) OS_dispatch_queue * _Nonnull handleQueue;
/**
...
/**
  If \code
  true
  \endcode, this client will try and reconnect on any disconnects.
*/
...
/**
  ...
  If changed after calling \code
  init
  \endcode, \code
  forceNew
  \endcode must be set to \code
  true
  \endcode, or it will only connect to the url set in the
  init.
*/
...
- (void)connect;
/**
  Connect to the server. If we aren’t connected after \code
  timeoutAfter
  \endcode seconds, then \code
  withHandler
  \endcode is called.
...
/**
  ...
  \param with The items to send with this event. Use \code
  []
  \endcode to send nothing.
  ...
  An \code
  OnAckCallback
  \endcode. You must call the \code
  timingOut(after:)
  \endcode method before the event will be sent.
*/
...
/**
  ...
  \param isInternalMessage If \code
  true
  \endcode event handlers for this event will be called regardless of status.
...
/**
  ...
  namespace
  \endcode.
  <em>Do not use this to join the default namespace.</em> Instead call \code
  leaveNamespace
  \endcode.
...
/**
  ...
  If you wish to remove a specific event, call the \code
  off(id:)
  \endcode with the UUID received from its \code
  on
  \endcode call.
...
/**
  ...
  once
  \endcode
  If you want to remove all events for an event, call the off \code
  off(_:)
  \endcode method with the event name.
...
/**
  ...
  This will cause a \code
  disconnect
  \endcode event to be emitted, as well as an \code
  reconnectAttempt
  \endcode event.
*/
- (void)reconnect;
