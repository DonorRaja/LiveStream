...
class VideoViewController: UIViewController, WZStatusCallback, WZVideoSink, WZAudioSink {
    ...
    let SDKSampleAppLicenseKey = "GOSK-5944-0103-6379-09DC-6B9B"
    ...
    lazy var session: LFLiveSession = {
        ...
        session.preView = self.goCoder?.cameraView
        return session
    }()
    ...
    func timerUpdate(){
        ...
        counter += 1
        self.timerCountLabel.text = String(format: "%02d:%02d:%02d", counter / 3600, (counter % 3600) / 60, (counter % 3600) % 60)
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "chatView" {
            chatView = segue.destination as! ChatViewController
            chatView.socket = socket
            self.createRoom()
        }
    }
    
    func liveStreamUpdate(broadcastImage:String,cameraIsEnable:Bool){
        self.broadcastButton.setImage(UIImage(named: broadcastImage), for: UIControlState())
        self.updateUIControls()
       // self.switchCameraButton.isEnabled = cameraIsEnable
    }
...
//        })
    ...
    func updateUIControls() {
        if self.goCoder?.status.state != .idle && self.goCoder?.status.state != .running {
           ...
           // self.switchCameraButton.isEnabled = !isStreaming
    ...
    func onWZStatus(_ status: WZStatus!) {
        ...
        case .idle:
            DispatchQueue.main.async { () -> Void in
                self.timerReset()
                self.liveStreamUpdate(broadcastImage: "start_button", cameraIsEnable: false)
        ...
        case .running:
            DispatchQueue.main.async { () -> Void in
                self.timerStart()
                self.liveStreamUpdate(broadcastImage: "stop_button", cameraIsEnable: true)
                self.containerViewHeight.constant = 44
            }
